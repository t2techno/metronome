...
const mult_matrix = ([[a, b], [c, d]]: number[][], [x, y]: number[]) => [
  a * x + b * y,
  c * x + d * y,
];
const rotate_matrix = (x: number) => [
  [cos(x), -sin(x)],
  [sin(x), cos(x)],
];
...
export const svgEllipseArc = (
  [cx, cy]: number[],
  [rx, ry]: number[],
  [startingAngle, sweepAmount]: number[],
  phi: number
) => {
  let arcString = "";
  ...
  const [startX, startY] = add_vec(
    mult_matrix(rotMatrix, [rx * cos(startingAngle), ry * sin(startingAngle)]),
    [cx, cy]
  );
  const [eX, eY] = add_vec(
    mult_matrix(rotMatrix, [
      rx * cos(startingAngle + sweepAmount),
      ry * sin(startingAngle + sweepAmount),
    ]),
    [cx, cy]
  );
  ...
  arcString +=
    "A " +
    [rx, ry, globalRotation, largSmallArcFlag, sweepDirectionFlag, eX, eY].join(
      " "
    );
  return arcString;
};

export const svgCircleArc = (
  c: number,
  r: number,
  [startingAngle, sweepRadians]: number[],
  globalRotationPercent: number
) => {
  return svgEllipseArc(
    [c, c],
    [r, r],
    [startingAngle, TWO_PI * sweepRadians],
    globalRotationPercent
  );
};

const INIT_ROT = (3 * PI) / 4;
const TOTAL_CIRCLE_PERCENT = 0.25;
export const volumeArc = (c: number, r: number, sweepPercentage: number) => {
  const sweepRad = PI * sweepPercentage * TOTAL_CIRCLE_PERCENT;
  return svgCircleArc(c, r, [0, sweepRad], INIT_ROT);
};
...
export const getAngle = (p1: Point, p2: Point) => {
  ...
  // up left
  if (p1.y <= p2.y && p1.x >= p2.x) return asin(distY / dist);
  // up right
  else if (p1.y <= p2.y && p1.x < p2.x) return PI / 2 + asin(distX / dist);
  // bottom right
  else if (p1.y > p2.y && p1.x < p2.x) return PI + asin(distY / dist);
